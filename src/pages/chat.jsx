...
export default class WeChat extends React.Component {
 ...
 sendmessage() {
    if (this.state.message) {
      io.emit('sendMsg', { name: this.state.user, message:this.state.message });
        ...
        <ul className={style.messages} id="mes">
          {this.state.list.map((item, index) => (
            <li
              key={index}
              className={
                this.state.user == item.name ? style.chartlistright : style.chartlistleft
              }
            >
